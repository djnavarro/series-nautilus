...
make_art <- function(seed, name, version) {
  ...
  width_scale <- runif(1, min = .2, max = .4)
  length_scale <- runif(1, min = .1, max = .2)
  curl_scale <- .2
  ...
  base <- tibble::tibble(
    ...
    r0 = a * th^k,
    r = 1.5 * r0 / max(r0),
    x_jtr = rcauchy(n_ribbons, 0, .02) * r,
    y_jtr = rcauchy(n_ribbons, 0, .02) * r,
    x = r * cos(th) + x_jtr,
    y = r * sin(th) + y_jtr,
    sh = runif(n_ribbons, .75, 1.25) * length_scale,
    x2 = r * cos(th + sh) + x_jtr,
    y2 = r * sin(th + sh) + y_jtr,
  )
