...
make_art <- function(seed, name, version) {
  ...
  length_scale <- runif(1, min = 1, max = 2) * .1
  curl_scale <- .2
  cauchy_scale <- .1
  ...
  base <- tibble::tibble(
    th = rbeta(n_ribbons, 10, 10) * n_wrap * 2 * pi,
    r0 = a * th^k,
    r = 1.5 * r0 / max(abs(r0)),
    x_jtr = rcauchy(n_ribbons, 0, cauchy_scale) * r,
    y_jtr = rcauchy(n_ribbons, 0, cauchy_scale) * r,
    ...
    x_mid = runif(n_ribbons, min = -1, max = 1) * .1,
    y_mid = runif(n_ribbons, min = -1, max = 1) * .1
  )
